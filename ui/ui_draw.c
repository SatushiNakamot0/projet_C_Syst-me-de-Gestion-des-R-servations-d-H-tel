...
void ui_draw_header(UIContext *ctx, Layout *layout) {
    ...
    attron(ui_theme_get_pair(COLOR_PAIR_HEADER));
    for (int i = 0; i < w; i++) {
        mvaddch(y, x + i, ' ');
    }
    
    /* Hotel name */
    mvprintw(y, x + 2, "HOTEL MANAGEMENT SYSTEM");
    ...
    mvprintw(y, w - 30, "%s | %s", date_str, time_str);
    
    attroff(ui_theme_get_pair(COLOR_PAIR_HEADER));
}
...
void ui_draw_dashboard(UIContext *ctx, Layout *layout) {
    ...
    float occupancy_rate = ctx->chambres_count > 0 ? 
        (occupied_rooms * 100.0f / ctx->chambres_count) : 0.0f;
...
void ui_draw_clients_list(UIContext *ctx, Layout *layout) {
    ...
    if (end_idx > ctx->clients_count) end_idx = ctx->clients_count;
...
void ui_draw_rooms_list(UIContext *ctx, Layout *layout) {
    ...
    int start_idx = ctx->scroll_offset;
    int end_idx = start_idx + visible_rows;
    if (end_idx > ctx->chambres_count) end_idx = ctx->chambres_count;
    ...
    for (int i = 0; i < ctx->chambres_count; i++) {
        if (ctx->chambres[i].disponible) disponibles++;
    }
...
void ui_draw_rooms_add(UIContext *ctx, Layout *layout) {
    ...
    mvprintw(y + 3, x + 2, "Room form will be displayed here...");
}
...
void ui_draw_reservations_list(UIContext *ctx, Layout *layout) {
    ...
    int start_idx = ctx->scroll_offset;
    int end_idx = start_idx + visible_rows;
    if (end_idx > ctx->reservations_count) end_idx = ctx->reservations_count;
...
void ui_draw_billing_list(UIContext *ctx, Layout *layout) {
    ...
    int start_idx = ctx->scroll_offset;
    int end_idx = start_idx + visible_rows;
    if (end_idx > ctx->factures_count) end_idx = ctx->factures_count;
...
void ui_draw_status_message(UIContext *ctx, Layout *layout) {
    ...
    switch (ctx->status_type) {
        case 1: color_pair = COLOR_PAIR_SUCCESS; break;
        case 2: color_pair = COLOR_PAIR_WARNING; break;
        case 3: color_pair = COLOR_PAIR_ERROR; break;
        default: color_pair = COLOR_PAIR_INFO; break;
    }
