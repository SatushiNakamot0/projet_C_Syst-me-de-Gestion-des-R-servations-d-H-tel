...
void ui_component_table_draw(TableComponent *table,
    ...
    if (end_idx > table->total_rows) end_idx = table->total_rows;
...
bool ui_component_table_handle_input(TableComponent *table, int key) {
    if (!table || !table->has_focus) return false;
    ...
    switch (key) {
        ...
        case KEY_PPAGE:
            ...
            if (table->selected_row < 0) table->selected_row = 0;
            table->scroll_offset = table->selected_row;
            return true;
...
void ui_component_input_draw(const InputFieldComponent *field) {
    ...
    attroff(ui_theme_get_pair(field->has_focus ? COLOR_PAIR_INPUT_FOCUS :
                              field->is_error ? COLOR_PAIR_ERROR :
                              COLOR_PAIR_INPUT));
...
bool ui_component_input_handle_input(InputFieldComponent *field, int key) {
    if (!field || !field->has_focus || !field->buffer) return false;
...
bool ui_component_button_handle_input(ButtonComponent *btn, int key) {
    if (!btn || btn->is_disabled) return false;
...
void ui_component_status_draw(const StatusMessageComponent *status) {
    ...
    ColorPair color_pair;
    switch (status->type) {
        case STATUS_SUCCESS: color_pair = COLOR_PAIR_SUCCESS; break;
        case STATUS_WARNING: color_pair = COLOR_PAIR_WARNING; break;
        case STATUS_ERROR: color_pair = COLOR_PAIR_ERROR; break;
